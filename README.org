...
To see more details, check ~tonyaldon.cln.rpc.core/call~ docstring.
...
in the namespace ~tonyaldon.cln.rpc.core~ with the same name of the
builtin methods.
...
library namespace too: ~clnrpc-clj~ -> ~tonyaldon.cln.rpc.core~.
